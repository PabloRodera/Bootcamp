...
class UserControllerTest {
    ...
    void signup_ValidRequest_ReturnsOkResponse(){
        SignUpRequest request = new SignUpRequest("username", "tester@example.com", "password");

        when(authenticationService.signup(any(SignUpRequest.class))).thenCallRealMethod();

        ResponseEntity<?> responseEntity = userController.signup(request);
    ...
    void signup_InvalidRequest_ReturnsBadRequestResponse(){
        SignUpRequest request = new SignUpRequest(null, "tester@example.com", null);
    ...
    @Test
    void login_ValidRequest_ReturnsOkResponse() throws Exception {
